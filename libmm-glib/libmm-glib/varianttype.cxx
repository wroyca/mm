...
namespace Glib
{
  ...
  const VariantType
      VARIANT_TYPE_OBJECT_PATH_ARRAY (G_VARIANT_TYPE_OBJECT_PATH_ARRAY);

  const VariantType VARIANT_TYPE_BYTESTRING (G_VARIANT_TYPE_BYTESTRING);

  const VariantType
      VARIANT_TYPE_BYTESTRING_ARRAY (G_VARIANT_TYPE_BYTESTRING_ARRAY);

  auto
  VariantType::get_string () const -> std::string
  {
    return std::string (g_variant_type_peek_string (gobj ()),
                        g_variant_type_get_string_length (gobj ()));
  }

  auto
  VariantType::get_item_types () const -> std::vector<VariantType>
  {
  ...
  auto
  Value<VariantType>::value_type () -> GType
  {
    return G_TYPE_VARIANT_TYPE;
  }

  auto
  Value<VariantType>::set (const CppType& data) -> void
  {
    set_boxed (data.gobj ());
  }

  auto
  Value<VariantType>::get () const -> CppType
  {
...
namespace Glib
{
  ...
  auto
  wrap (GVariantType* object, const bool take_copy) -> VariantType
  {
...
namespace Glib
{
  ...
  VariantType::VariantType (GVariantType* castitem, const bool make_a_copy)
  {
    if (!make_a_copy)
    {
      gobject_ = castitem;
  ...
  auto
  VariantType::operator= (const VariantType& src) -> VariantType&
  {
    const auto new_gobject = src.gobject_ ? g_variant_type_copy (src.gobject_) :
                                            nullptr;
  ...
  auto
  VariantType::operator= (VariantType&& other) noexcept -> VariantType&
  {
  ...
  auto
  VariantType::swap (VariantType& other) noexcept -> void
  {
    std::swap (gobject_, other.gobject_);
  }

  auto
  VariantType::gobj_copy () const -> GVariantType*
  {
    return g_variant_type_copy (gobject_);
  }

  auto
  VariantType::create_array (const VariantType& element) -> VariantType
  {
    return wrap (g_variant_type_new_array (element.gobj ()));
  }

  auto
  VariantType::create_maybe (const VariantType& element) -> VariantType
  {
    return wrap (g_variant_type_new_maybe (element.gobj ()));
  }

  auto
  VariantType::create_dict_entry (const VariantType& key,
  ...
  auto
  VariantType::get_string_length () const -> gsize
  {
  ...
  auto
  VariantType::is_container () const -> bool
  {
    return g_variant_type_is_container (gobj ());
  }

  auto
  VariantType::is_basic () const -> bool
  {
    return g_variant_type_is_basic (gobj ());
  }

  auto
  VariantType::is_maybe () const -> bool
  {
    return g_variant_type_is_maybe (gobj ());
  }

  auto
  VariantType::is_array () const -> bool
  {
    return g_variant_type_is_array (gobj ());
  }

  auto
  VariantType::is_tuple () const -> bool
  {
    return g_variant_type_is_tuple (gobj ());
  }

  auto
  VariantType::is_dict_entry () const -> bool
  {
    return g_variant_type_is_dict_entry (gobj ());
  }

  auto
  VariantType::is_variant () const -> bool
  {
    return g_variant_type_is_variant (gobj ());
  }

  auto
  VariantType::hash () const -> guint
  {
    return g_variant_type_hash (gobj ());
  }

  auto
  VariantType::equal (const VariantType& other) const -> bool
  {
    return g_variant_type_equal (gobj (), other.gobj ());
  }

  auto
  VariantType::is_subtype_of (const VariantType& supertype) const -> bool
  {
    return g_variant_type_is_subtype_of (gobj (), supertype.gobj ());
  }

  auto
  VariantType::element () const -> VariantType
  {
    return wrap (const_cast<GVariantType*> (g_variant_type_element (gobj ())),
                 true);
  }

  auto
  VariantType::n_items () const -> gsize
  {
    return g_variant_type_n_items (gobj ());
  }

  auto
  VariantType::key () const -> VariantType
  {
    return wrap (const_cast<GVariantType*> (g_variant_type_key (gobj ())),
                 true);
  }

  auto
  VariantType::value () const -> VariantType
  {
    return wrap (const_cast<GVariantType*> (g_variant_type_value (gobj ())),
                 true);
  }
